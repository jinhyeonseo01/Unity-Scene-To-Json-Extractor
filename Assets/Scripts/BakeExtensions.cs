...
public class BakeExtensions
{
    public static JToken ToJson(object obj)
    {
        ...
        if (obj is Material)
        {
            ...
            materialJson.Add("guid", SceneBaking.hash2guidTable[material.GetHashCode()]);
            ...
            if (!string.IsNullOrEmpty(path))
            {
                switch (path)
                {
                    case "Library/unity default resources":
                        {
                            if(!SceneBaking.nameToPath.ContainsKey(mesh.name))
                                SceneBaking.nameToPath[mesh.name] = mesh.name + ".fbx";
                            
                            path = SceneBaking.definePath_Resources + SceneBaking.nameToPath[mesh.name];
                            break;
                        }
                    default:
                        {
